...
import org.powershell.editors.completition.dictionaries.PowershellDictionary;
import org.powershell.editors.syntax.IPSColorConstants;
...
public class PowershellScanner extends RuleBasedScanner {
	...
	// static {
	// try {
	// known_functions = InterFileDictionary.getInstance().getForColor();
	//
	// } catch (Exception e) {
	// e.printStackTrace();
	// }
	...
	public PowershellScanner() {
		...
		IToken comment = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.COMMENT)));
		IToken string = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.STRING)));
		IToken variable = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.VARIABLE)));
		IToken scope = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.VARIABLE), null,
				SWT.BOLD));
		IToken keyword = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.LANGUAGE_RESERVED),
				null, SWT.BOLD));
		IToken cmdlet = new Token(
				new TextAttribute(new Color(Display.getCurrent(),
						IPSColorConstants.CMDLET), null, SWT.BOLD));
		IToken known_func = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.KNOWN_FUNC), null,
				SWT.BOLD));
		IToken dashKeyword = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.DASH_OPERATOR), null,
				SWT.BOLD));
		IToken def = new Token(new TextAttribute(new Color(
				Display.getCurrent(), IPSColorConstants.DEFAULT)));
